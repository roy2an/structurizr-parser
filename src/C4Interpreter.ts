...
class c4Interpreter extends BaseStructurizrVisitor {
    ...
    personSection(node: any, group?: C4Group) {
        ...
        const person = new C4Person(id, name, description, tags);
        if (group){
    ...
    softwareSystemSection(node: any, group?: C4Group) {
        ...
        const description = stripQuotes(node.StringLiteral[1]?.image ?? "");
        const tags = stripQuotes(node.StringLiteral[2]?.image ?? "");
        const ssys = new C4SoftwareSystem(id, name, description, tags);
        if (group){
    ...
    containerSection(node: any, ssys:C4SoftwareSystem) {
        ...
        const description = stripQuotes(node.StringLiteral[1]?.image ?? "");
        const tags = stripQuotes(node.StringLiteral[2]?.image ?? "");
        const container = new C4Container(id, name, description, tags);
    ...
    componentSection(node: any, ctr: C4Container) {
        ...
        const description = stripQuotes(node.StringLiteral[1]?.image ?? "");
        const tags = stripQuotes(node.StringLiteral[2]?.image ?? "");
        const component = new C4Component(id, name, description, tags);
    ...
    explicitRelationship(node: any, ctr: C4Container) {
        ...
        const tech = stripQuotes(node.StringLiteral[1]?.image ?? "");
        const tags = stripQuotes(node.StringLiteral[2]?.image ?? "");
        this.workspace.addRelationship(s_id, t_id, desc, tech, tags);
    }
